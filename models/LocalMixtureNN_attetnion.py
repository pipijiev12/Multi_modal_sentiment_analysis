...
class LocalMixtureNN_attention(torch.nn.Module):
    def __init__(self, opt):
        ...
        """
        super(LocalMixtureNN_attention, self).__init__()
...
#        self.hidden_units = opt.hidden_units
    ...
    def forward(self, in_modalities):
        ...
        # print('probs_tensor',np.shape(probs_tensor))
        probs_feature = []
        for one_type in self.pooling_type.split(','):
            one_type = one_type.strip()
            if one_type == 'max':
#                probs = GlobalMaxPooling1D()(self.probs)
        ...
        # print('probs_feature',np.shape(probs_feature[0]))
