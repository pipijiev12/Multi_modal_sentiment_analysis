...
class LocalMixtureNN_attention_V(torch.nn.Module):
    def __init__(self, opt):
        ...
        """
        super(LocalMixtureNN_attention_V, self).__init__()
...
#                    pre_trained_phases = pickle.load(open(pkl_path,'rb'))
            if ind==0:
                # print('opt',opt.lookup_table.shape[0])
...
#        self.hidden_units = opt.hidden_units
        ...
        # self.bilinear = nn.Bilinear(50, 50, 50)
        #
        self.real_softmax = nn.Softmax(dim=1)
        self.imag_softmax = nn.Softmax(dim=1)
        #
        # # self.weight = torch.rand(1).to(self.device)
        #
        # self.real_linear = nn.Linear(1000,1000)
        # self.imag_linear = nn.Linear(1000,1000)
        self.linear_1 = nn.Linear(10,1000)
        self.linear_2 = nn.Linear(10,1000)
        self.linear_3 = nn.Linear(10,1000)
    def forward(self, in_modalities):
        ...
        # print('in_modalities[0]', np.shape(in_modalities[0]))
        # print('in_modalities[1]', np.shape(in_modalities[1]))
        # print('in_modalities[2]', np.shape(in_modalities[2]))

        # print(i,ind)
        # print('word_indexes',np.shape(word_indexes))
        phases = self.phase_embed[1](word_indexes)
        # print('phases',np.shape(phases))
        amplitudes = self.proj_layers[1](in_modalities[1])
        # print('amplitudes', np.shape(amplitudes))
        ...
        # print('real_tensors',np.shape(real_tensors))
        # print('real_tensors-1',np.shape(real_tensors[0]))
        # print(np.shape(real_tensors[0]),'------------')
        # print(np.shape(imag_tensors[0]),'++++++++++++')
        real_tensors = torch.stack(real_tensors,dim = 1)
        imag_tensors = torch.stack(imag_tensors,dim = 1)
        # print('real_tensors',np.shape(real_tensors))
        # print('imag_tensors',np.shape(imag_tensors))
        ...
        # 双线性
        # print('U_tensors_real',np.shape(U_tensors_real))
        # print('U_tensors_imag',np.shape(U_tensors_imag))
        # U_tensors_real = self.bilinear(U_tensors_real,U_tensors_real) - self.bilinear(U_tensors_imag,U_tensors_imag)
        # U_tensors_imag = self.bilinear(U_tensors_imag,U_tensors_real) + self.bilinear(U_tensors_real,U_tensors_imag)
        # print('U_tensors_real',np.shape(U_tensors_real))
        # print('U_tensors_imag',np.shape(U_tensors_imag))
        ...
        # atten_real_tensors = self.real_linear(nn.Dropout(0.2)(atten_real_tensors))
        # atten_imag_tensors = self.imag_linear(nn.Dropout(0.2)(atten_imag_tensors))


        real_tensors = atten_real_tensors + Q_tensors_real
        imag_tensors = atten_imag_tensors + Q_tensors_imag
        # print(np.shape(real_tensors), '==================')

        # real_tensors = torch.mul(atten_real_tensors,self.weight) + torch.mul(Q_tensors_real,1-self.weight)
        # imag_tensors = torch.mul(atten_imag_tensors,self.weight) + torch.mul(Q_tensors_imag,1-self.weight)



        prob_list = []
        # print('self.ngram',self.ngram)

        n_gram_embedding_real = self.ngram[0](real_tensors)
        n_gram_embedding_imag = self.ngram[0](imag_tensors)
        n_gram_weight = self.ngram[0](weight)
        n_gram_weight = self.activation(n_gram_weight)
        n_gram_embedding_real = self.linear_1(n_gram_embedding_real)
        n_gram_embedding_imag = self.linear_2(n_gram_embedding_imag)
        # print(np.shape(n_gram_embedding_real))
        # print(np.shape(n_gram_embedding_imag))
        # print(np.shape(n_gram_weight))
...
#        for dim in self.contracted_dims
    ...
    def get_submodality_results(self, in_modalities, feature_indexes):
        ...
        amplitudes = self.proj_layers[0](in_modalities[0])
        norms = self.l2_norm(amplitudes)
        amplitudes = self.l2_normalization(amplitudes)
        ...
        U_tensors_real = torch.bmm(Q_tensors_real, K_tensors_real.transpose(1, 2)) - torch.bmm(Q_tensors_imag,
                                                                                               K_tensors_imag.transpose(
                                                                                                   1, 2))
        U_tensors_imag = torch.bmm(Q_tensors_imag, K_tensors_real.transpose(1, 2)) + torch.bmm(Q_tensors_real,
                                                                                               K_tensors_imag.transpose(
                                                                                                   1, 2))
        # 缩放点积
        U_tensors_real = U_tensors_real / math.sqrt(np.shape(K_tensors_real)[2])
        ...
        n_gram_embedding_real = self.ngram[0](real_tensors).detach().cpu().numpy()
        n_gram_embedding_imag = self.ngram[0](imag_tensors).detach().cpu().numpy()
        n_gram_weight = self.ngram[0](weight)
...
#                    dm = np.zeros(self.measurement_dim, dtype = 'complex64')
                ...
                for proj in projector_list:
                    prob = 0
                    for w_id in range(self.ngram[0].gram_n):
